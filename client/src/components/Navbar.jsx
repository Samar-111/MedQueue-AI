      ...
      <div className="max-w-7xl mx-auto glass-dock rounded-full px-6 py-3 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setActiveView('intake')}>
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-600 font-bold mt-0.5">Emergency Intake & Priority Queue</p>
          </div>
        </div>

        <nav className="flex items-center gap-1.5 sm:gap-2 bg-slate-200/70 backdrop-blur-md p-1.5 rounded-full border border-slate-300/80 shadow-inner">
          {views.map((tab) => {
            const Icon = tab.icon;
              ...
              <button
                ...
                className={`relative flex items-center gap-2 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-full text-xs sm:text-sm font-extrabold transition-all duration-200 cursor-pointer ${
                  isActive
                    ...
                    : 'bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 text-slate-900 hover:scale-[1.02] shadow-sm'
                }`}
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-white' : 'text-slate-700'}`} />
                <span className={`tracking-tight font-extrabold ${isActive ? 'text-white' : 'text-slate-900'}`}>
                  {tab.label}
                </span>
                {tab.badge !== null && tab.badge > 0 && (
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${
                    isActive ? 'bg-white text-orange-600 shadow-sm' : 'bg-slate-200 text-slate-900 font-black shadow-sm'
                  }`}>
                    {tab.badge}
                  </span>
                )}
        ...
        <div className="flex items-center gap-3">
          {criticalCount > 0 && (
            <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-rose-800 text-xs font-black shadow-sm animate-pulse">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-white hover:bg-slate-50 border border-slate-200 text-xs font-extrabold text-slate-800 hover:text-orange-600 transition-all duration-200 hover:scale-105 shadow-sm cursor-pointer"
          >
